import os, time, threading, queue, json, signal, sys
...
ROBOT_SIZE = int(os.getenv("ROBOT_SIZE, 50"))
PERIOD_MS = int(os.getenv("PERIOD_MS", 1000))
...
# Quue & Writer Therad
q = queue.Queue(maxsize=20000)              # Queue -> pandas -> executemany Batch
...
def db_writer():
    ...
    while True:
        ...
        if len(buffer) >= 1000:
            ...
            # 예시 : 실시간 벡터 평균 계산(Numpy) - 필요없으면 제거
...
# 로봇 수집 스레드
def robot_worker(idx: int, ip: str, prot: int):
    label = f"roibot{idx}"
    indy = IndyDCP2(host = ip, port = port)
    indy.connect()
    print(f"[{lable}] connected")
    while True:
        try:
            jt = indy.get_joint_torque() # 로봇 토크
            pose = indy.read_tcp_pose() # TCP 좌표
            state = indy.read_robot_state() # 로봇 상태
            run_time = indy.get_robot_running_time() # 로봇 실행 시간
...
def plc_worker():
    ...
    mc.setaccessopt(
        ...
        iServerPort = int(os.getenv("PLC_PORT", 4999))
    )
    start_d = os.getenv("PLC_START_DEVICE", "D100")
    words = int(os.getenv("PLC_WORDS", 31))
    while True:
        try:
            data = mc.batchread_random(word_devices=[f"{start_d}{i}"for i in range(words)])     # 예시 : 첫 세 word만 저장
            q.put({
...
for i in range(1, ROBOT_COUNT + 1):
    ...
    port = int(os.getenv(f"ROBOT{i}_PORT", 6066))
...
def graceful_exit(signum, frame):
    print("Stopping...")
    q.put(None)  # Writer Thread 종료 신호
